...
int main(int argc, char** argv) {
  ...
  for (uint64_t epoch = 1; epoch < iterations + 1; epoch++) {
    ...
    // 2. signal readiness to receive data
    for (int i = 0; i < 6; i++) {
      res =
    ...
    // 3. wait for readiness from neighbors
    ...
    // 4. put data to neighbors
    for (int i = 0; i < 6; i++) {
      res = foMPI_Put_notify(surface_data_in[i], N * N, MPI_DOUBLE,
                             neighbors[i],
                             remote_offset[i] * N * N, N * N, MPI_DOUBLE,
                             data_win,
                             i);
      assert(res == 0);
    }
