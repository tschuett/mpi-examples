...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    // put xminus
    res = MPI_Put(data[0], message_size, MPI_DOUBLE_COMPLEX, xminus, 0,
                  message_size, MPI_DOUBLE_COMPLEX, win);
    assert(res == 0);
    // put xplus
    res = MPI_Put(data[1], message_size, MPI_DOUBLE_COMPLEX, xplus, 0,
                  message_size, MPI_DOUBLE_COMPLEX, win);
    assert(res == 0);
    ...
    // put yminus
    res = MPI_Put(data[2], message_size, MPI_DOUBLE_COMPLEX, yminus, 0,
                  message_size, MPI_DOUBLE_COMPLEX, win);
    assert(res == 0);
    // put yplus
    res = MPI_Put(data[3], message_size, MPI_DOUBLE_COMPLEX, yplus, 0,
                  message_size, MPI_DOUBLE_COMPLEX, win);
    assert(res == 0);
    ...
    // put zminus
    res = MPI_Put(data[4], message_size, MPI_DOUBLE_COMPLEX, zminus, 0,
                  message_size, MPI_DOUBLE_COMPLEX, win);
    assert(res == 0);
    // put zplus
    res = MPI_Put(data[5], message_size, MPI_DOUBLE_COMPLEX, zplus, 0,
                  message_size, MPI_DOUBLE_COMPLEX, win);
    assert(res == 0);
